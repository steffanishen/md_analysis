...
ANALYSIS_RANDOM_WALK::ANALYSIS_RANDOM_WALK(PSF *system, GROUP *sel1, int vector1d, int vector2d, int voidf, float bl, string filename, vector<PSF*> monomers, float fringe) //constructor
{
    this->system = system;
    this->sel1 = sel1;
...
void ANALYSIS_RANDOM_WALK::init() {
}


vector<vector<float>> ANALYSIS_RANDOM_WALK::bond_next(int residueid, PSF *monomer) {
...
vector<vector<float>> ANALYSIS_RANDOM_WALK::bond_pre(int residueid, PSF *monomer) {
...
void ANALYSIS_RANDOM_WALK::compute_void() {
    FILE * fpdb;
  ...
  //  vector<float> next_bond(3);
    ...
    cout << "Box size: " << system->pbc[0] << " " << system->pbc[2] << " " << system->pbc[5] << endl;
...
ANALYSIS_RANDOM_WALK::~ANALYSIS_RANDOM_WALK()
{
    system = NULL;
    sel1 = NULL;
}
