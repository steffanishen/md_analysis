...
//    sel1 = sys.select_atoms("resname IMP and atomtype NE");
    ...
    DCD_R dcdf(dcdname,&sys); //MS changed: the first argument specifies the dcd file name. Note this dcdf is a DCD_R object, while dcdf in dcdr.hpp is a fstream object.
    dcdf.read_header();
    cout << "finished reading dcd header!!" << endl;
    dcdf.printHeader();


    cout << "Number of frames: " << dcdf.getNFILE() << endl;
...
//////////////////// We save output for 2D vectors for later ///////////////////////////
	    ...
	    vector<float> mindA_seg = analysis[compID]->compute_vector();
	    *files[compID] << "frame: " << i << " angles: ";
            for (int segi = 0; segi < mindA_seg.size(); segi++) {
	        *files[compID] << mindA_seg[segi] << " ";
	    }
	    *files[compID] << endl;
